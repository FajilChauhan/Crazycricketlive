...
export const dashboardService = {
  ...
  getTopPlayers: async () => {
    try {
      const [battingResult, bowlingResult] = await Promise.all([
        pool.query(
          `SELECT bb.striker_id AS user_id,
                  u.username,
                  COALESCE(SUM(bb.runs_scored), 0)::int AS total_runs
           FROM ball_by_ball bb
           LEFT JOIN users u ON bb.striker_id = u.user_id
           WHERE bb.striker_id IS NOT NULL
        ...
        pool.query(
          `SELECT bb.bowler_id AS user_id,
                  u.username,
                  COALESCE(SUM(CASE WHEN bb.is_wicket = true THEN 1 ELSE 0 END), 0)::int AS total_wickets
           FROM ball_by_ball bb
      ...
      return {
        topBatters: battingResult.rows,
        topBowlers: bowlingResult.rows,
      };
    } catch (error) {
